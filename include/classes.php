...
class mf_custom_login
{
	...
	function check_if_logged_in($data = array())
	{
		...
		if(is_user_logged_in())
		{
			...
			else
			{
				...
				$done_text = sprintf(__("You are already logged in as %s. Would you like to go to %sadmin%s or %slog out %s?", $this->lang_key), $user_data->user_login, "<a href='".admin_url()."'>", "</a>", "<a href='".wp_logout_url()."'>", "</a>");
			}
		}
	}
	...
	function settings_custom_login()
	{
		...
		if($has_login_widget == false && $has_registration_widget == false && $has_lost_password_post_widget == false)
		{
			...
			{
				$arr_settings['setting_custom_login_display_theme_logo'] = __("Display Theme Logo", $this->lang_key);
			}

			if(get_option('setting_custom_login_display_theme_logo') != 'yes')
			{
				$arr_settings['setting_custom_login_custom_logo'] = __("Custom Logo", $this->lang_key);
			}
		}
		...
		{
			$arr_settings['setting_custom_login_allow_direct_link'] = __("Allow Direct Link to Login", $this->lang_key);

			if(get_option('setting_custom_login_allow_direct_link') == 'yes')
			{
				$arr_settings['setting_custom_login_direct_link_expire'] = __("Direct Link Expires After", $this->lang_key);
				$arr_settings['setting_custom_login_direct_link_expire_after_login'] = __("Direct Link Expires After Login", $this->lang_key);
			}
		}
		...
		{
			$arr_settings['setting_custom_login_allow_api'] = __("Allow API Login", $this->lang_key);

			if(isset($_SERVER['PHP_AUTH_USER']))
			{
				$arr_settings['setting_custom_login_allow_server_auth'] = __("Allow Server Authentication", $this->lang_key);
			}
		}
		...
		$arr_settings['setting_custom_login_limit_attempts'] = __("Limit Attempts", $this->lang_key);
		$arr_settings['setting_custom_login_limit_minutes'] = __("Limit Minutes", $this->lang_key);

		$arr_settings['setting_custom_login_redirect_after_login_page'] = __("Redirect After Login", $this->lang_key);

		if(get_option('setting_custom_login_redirect_after_login_page') > 0)
		{
			$arr_settings['setting_custom_login_redirect_after_login'] = __("Who to Redirect", $this->lang_key);
		}

		$arr_settings['setting_custom_login_debug'] = __("Debug", $this->lang_key);
		...
		if(is_multisite())
		{
			$arr_settings['users_can_register'] = __("Allow Registration", $this->lang_key);

			if($users_can_register)
			{
				$arr_settings['default_role'] = __("Default Role", $this->lang_key);

				if($has_registration_widget == false)
				{
					$arr_settings['setting_custom_login_register'] = __("Register", $this->lang_key);
				}
			}
		}
		...
		if($users_can_register)
		{
			$arr_settings['setting_custom_login_info'] = __("Information", $this->lang_key);

			$arr_settings['setting_custom_login_email_admin_registration'] = __("Registration Email Content to Admin", $this->lang_key);
			$arr_settings['setting_custom_login_email_registration'] = __("Registration Email Content", $this->lang_key);
		}
		...
		{
			$arr_settings['setting_custom_login_lostpassword'] = __("Lost Password", $this->lang_key);
			$arr_settings['setting_custom_login_recoverpassword'] = __("Recover Password", $this->lang_key);
		}

		$arr_settings['setting_custom_login_email_lost_password'] = __("Lost Password Email Content", $this->lang_key);
	...
	function settings_custom_login_callback()
	{
		...
		echo settings_header($setting_key, __("Login", $this->lang_key));
	}
		...
		function setting_custom_login_page_callback()
		{
			...
			echo show_select(array('data' => $arr_data, 'name' => $setting_key, 'value' => $option, 'suffix' => get_option_page_suffix(array('value' => $option)), 'description' => __("The content from this page is displayed next to the login screen", $this->lang_key)));
		}
		...
		function setting_custom_login_allow_direct_link_callback()
		{
			...
			echo show_select(array('data' => get_yes_no_for_select(), 'name' => $setting_key, 'value' => $option, 'suffix' => __("This will allow users to get a direct link to use instead of username and password", $this->lang_key)));
		}
		...
		function setting_custom_login_direct_link_expire_callback()
		{
			...
			echo show_textfield(array('type' => 'number', 'name' => $setting_key, 'value' => $option, 'xtra' => "min='0' max='240'", 'suffix' => __("minutes", $this->lang_key)." (".__("0 means never", $this->lang_key).")"));
		}
		...
		function setting_custom_login_allow_api_callback()
		{
			...
			if($option == 'yes')
			{
				...
				echo "<p>".sprintf(__("By sending a request to %s with %s and %s as %s you will get a success or failure as a response", $this->lang_key), "<code>".$api_url."</code>", "<code>user_login</code>", "<code>user_pass</code>", "<code>POST</code>")."</p>";
			}
		}
		...
		function setting_custom_login_info_callback()
		{
			...
			echo sprintf(__("To take advantage of dynamic data, you can use the following placeholders: %s", $this->lang_key), sprintf('<code>%s</code>', implode('</code>, <code>', $tags)));
		}
		...
		function setting_custom_login_redirect_after_login_callback()
		{
			...
			echo show_select(array('data' => get_roles_for_select(array('add_choose_here' => false, 'use_capability' => false)), 'name' => $setting_key."[]", 'value' => $option, 'description' => __("Users with these roles will be redirected after login. If none are chosen, all are affected", $this->lang_key)));
		}
	...
	function settings_custom_login_registration_callback()
	{
		...
		echo settings_header($setting_key, __("Login", $this->lang_key)." - ".__("Registration", $this->lang_key));
	}
		...
		function users_can_register_callback()
		{
			...
			{
				$xtra = " disabled";
				$description = sprintf(__("You can change this in the %sNetwork Settings%s", $this->lang_key), "<a href='".network_admin_url("settings.php")."'>", "</a>");
			}
			...
			if($option == 1)
			{
				...
				if(!($registration_widget > 0))
				{
					echo "<p class='display_warning'>"
						...
						.sprintf(__("You have not created a %spage for registration%s. Please do so and add the %sregistration widget%s to the page", $this->lang_key), "<a href='".admin_url("post-new.php?post_type=page")."'>", "</a>", "<a href='".admin_url("widgets.php")."'>", "</a>")
					."</p>";
				}
			}
		}
		...
		function setting_custom_login_register_callback()
		{
			...
			echo show_select(array('data' => $arr_data, 'name' => $setting_key, 'value' => $option, 'suffix' => get_option_page_suffix(array('value' => $option)), 'description' => __("The content from this page is displayed next to the register screen", $this->lang_key)));
		}

		function setting_custom_login_email_admin_registration_callback()
		{
			$setting_key = get_setting_key(__FUNCTION__);
			$option = get_option_or_default($setting_key, sprintf(__("A new user (%s) has been created", $this->lang_key), "[user_login]"));
		...
		{
			$setting_key = get_setting_key(__FUNCTION__);
			$option = get_option_or_default($setting_key, __("Username", $this->lang_key).": [user_login]\r\n\r\n"
			.__("To set your password, visit the following address", $this->lang_key).": [confirm_link]");
	...
	function settings_custom_login_password_callback()
	{
		...
		echo settings_header($setting_key, __("Login", $this->lang_key)." - ".__("Password", $this->lang_key));
	}

		function setting_custom_login_email_lost_password_callback()
		{
			$setting_key = get_setting_key(__FUNCTION__);
			$option = get_option_or_default($setting_key, __("Someone has requested a password reset for the following account.", $this->lang_key)."\r\n"
			.__("Username", $this->lang_key).": [user_login]\r\n\r\n"
			.__("If this was a mistake, just ignore this email and nothing will happen.", $this->lang_key)."\r\n\r\n"
			.__("To reset your password, visit the following address", $this->lang_key).": [confirm_link]");
		...
		function setting_custom_login_lostpassword_callback()
		{
			...
			echo show_select(array('data' => $arr_data, 'name' => $setting_key, 'value' => $option, 'suffix' => get_option_page_suffix(array('value' => $option)), 'description' => __("The content from this page is displayed next to the lost password screen", $this->lang_key)));
		}
		...
		function setting_custom_login_recoverpassword_callback()
		{
			...
			echo show_select(array('data' => $arr_data, 'name' => $setting_key, 'value' => $option, 'suffix' => get_option_page_suffix(array('value' => $option)), 'description' => __("The content from this page is displayed next to the recover password screen", $this->lang_key)));
		}
	...
	function user_row_actions($actions, $user)
	{
		if(get_option('setting_custom_login_allow_direct_link') == 'yes' && current_user_can('edit_user') && isset($user->roles[0]) && $user->roles[0] != '')
		{
			...
			if($meta_login_auth != '')
			{
				$actions['direct_link'] = "<a href='".$this->direct_link_url(array('key' => $meta_login_auth, 'user_meta_exists' => true, 'user_data' => $user, 'type' => 'users'))."'>".__("Direct Link", $this->lang_key)."</a>";
			}
		}
	...
	function edit_user_profile($user)
	{
		if(IS_ADMIN && get_option('setting_custom_login_allow_direct_link') == 'yes')
		{
			echo "<table class='form-table'>
				<tr>
					<th><label>".__("Direct Link", $this->lang_key)."</label></th>
					<td>";
						...
						if($meta_login_auth != '')
						{
							echo "<p><a href='".$this->direct_link_url(array('key' => $meta_login_auth, 'user_meta_exists' => true, 'user_data' => $user, 'type' => 'profile'))."'>".__("URL", $this->lang_key)."</a></p>
							<div>"
								.show_button(array('type' => 'button', 'name' => 'btnDirectLoginRevoke', 'text' => __("Revoke", $this->lang_key), 'class' => "button-secondary", 'xtra' => "data-user-id='".$user->ID."'"))
							."</div>";
						}

						else
						{
							echo show_button(array('type' => 'button', 'name' => 'btnDirectLoginCreate', 'text' => __("Generate Now", $this->lang_key), 'class' => "button-secondary", 'xtra' => "data-user-id='".$user->ID."'"));
						}
			...
			switch(check_var('type'))
			{
				case 'link':
					...
					if($this->username != '' && $this->auth != '' && $this->check_auth())
					{
						...
						{
							$this->error = sprintf(__("I could not log in %s for you. If the problem persists, please contact an admin", $this->lang_key), $this->username);
						}
					}

					else
					{
						$this->error = sprintf(__("It looks like the authorization key for %s has expired so I can not let you in", $this->lang_key), $this->username);
					}
				break;
			}
		}
	...
	function direct_link_text($data)
	{
		...
		$out = __("To login directly without setting a password, visit the following link. The link is personal and can only be used once. If this link falls into the wrong hands and you have not used it they will be able to login to your account without a password.", $this->lang_key).":"
	...
	function login_form()
	{
		if(get_option('setting_custom_login_allow_direct_link') == 'yes')
		{
			echo "<p id='direct_login_link' class='hide'>
				<a href='#'>".__("Lost Password? Click to get a secure direct link to login instantly", $this->lang_key)."</a><br><br>
			</p>";
		}

		//echo show_textfield(array('name' => 'login_check', 'text' => __("This field should not be visible", $this->lang_key), 'xtra_class' => "form_check", 'xtra' => " autocomplete='off'"));
		echo wp_nonce_field('login_send_'.$_SERVER['REMOTE_ADDR'].'_'.date("Ymd"), '_wpnonce_login_send', true, false);
	}

	function register_form()
	{
		//echo show_textfield(array('name' => 'registration_check', 'text' => __("This field should not be visible", $this->lang_key), 'xtra_class' => "form_check", 'xtra' => " autocomplete='off'"));
	...
	{
		//echo show_textfield(array('name' => 'lost_password_check', 'text' => __("This field should not be visible", $this->lang_key), 'xtra_class' => "form_check", 'xtra' => " autocomplete='off'"));
	...
	function create_direct_login()
	{
		...
		if($user_id > 0)
		{
			if(IS_ADMIN && get_option('setting_custom_login_allow_direct_link') == 'yes')
			{
				...
				$result['success'] = true;
				$result['message'] = "<a href='".$this->direct_link_url(array('user_data' => $user_data, 'type' => 'profile'))."'>".__("URL", $this->lang_key)."</a>";
			}

			else
			{
				$result['error'] = __("You do not have the rights to perform this action", $this->lang_key);
			}
		}

		else
		{
			$result['error'] = __("There was no User ID attached to the request", $this->lang_key);
		}
	...
	function revoke_direct_login()
	{
		...
		if($user_id > 0)
		{
			if(IS_ADMIN && get_option('setting_custom_login_allow_direct_link') == 'yes')
			{
				...
				$result['success'] = true;
				$result['message'] = __("The direct login link has been revoked and can not be used anymore", $this->lang_key);
			}
	...
	function send_direct_link_email()
	{
		...
		if(isset($user->user_email) && $user->user_email != '')
		{
			...
			$mail_subject = sprintf(__("[%s] Here comes you link to direct login", $this->lang_key), get_bloginfo('name'));
			...
			{
				$result['success'] = true;
				$result['message'] = __("I successfully sent the message to your email. Follow the link in it, and you will be logged in before you know it", $this->lang_key);
			}

			else
			{
				$result['error'] = __("I could not send the email", $this->lang_key);
			}
		}

		else
		{
			$result['error'] = __("I could not find an email corresponding to the username you entered", $this->lang_key);
		}
...
class widget_login_form extends WP_Widget
{
	function __construct()
	{
		...
		$this->widget_ops = array(
			...
			'description' => __("Display a Login Form", $this->obj_custom_login->lang_key)
		);
		...
		parent::__construct('login-widget', __("Login Form", $this->obj_custom_login->lang_key), $this->widget_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		echo $before_widget;
			...
			switch($action)
			{
				case 'logout':
					if(is_user_logged_in())
					{
						...
						$done_text = __("You have been successfully logged out", $this->obj_custom_login->lang_key);
					}
				break;
				...
				default:
					...
					if(isset($_POST['btnSendLogin']))
					{
						...
						{
							$error_text = sprintf(__("You have exceeded the limit of %d logins in the last %d minutes. Please try again later.", $this->obj_custom_login->lang_key), $setting_custom_login_limit_attempts, $setting_custom_login_limit_minutes);
						}
					}
			...
			echo "<form method='post' action='".wp_login_url()."' class='mf_form'>"
				.show_textfield(array('name' => 'log', 'text' => __("Username or E-mail", $this->obj_custom_login->lang_key), 'value' => $user_login, 'placeholder' => "abc123 / ".get_placeholder_email(), 'required' => true))
				.show_password_field(array('name' => 'pwd', 'text' => __("Password"), 'value' => $user_pass, 'required' => true));

				do_action('login_form');

				echo "<div class='flex_flow'>"
					.show_checkbox(array('name' => 'rememberme', 'text' => __("Remember Me", $this->obj_custom_login->lang_key), 'value' => $user_remember))
					."<div class='form_button'>"
						.show_button(array('name' => 'btnSendLogin', 'text' => __("Log In", $this->obj_custom_login->lang_key)))
						.input_hidden(array('name' => 'redirect_to', 'value' => esc_attr($redirect_to)))
					."</div>
				</div>
				<p><a href='".wp_lostpassword_url().($user_login != '' ? "?user_login=".$user_login : '')."'>".__("Have you forgotten your login credentials?", $this->obj_custom_login->lang_key)."</a></p>";

				if(get_option('users_can_register'))
				{
					echo "<p>".__("Do not have an account?", $this->obj_custom_login->lang_key)." <a href='".wp_registration_url()."'>".__("Register", $this->obj_custom_login->lang_key)."</a></p>";
				}
	...
	function form($instance)
	{
		...
		echo "<div class='mf_form'>"
			.get_media_library(array('type' => 'image', 'name' => $this->get_field_name('login_image'), 'label' => __("Logo", $this->obj_custom_login->lang_key), 'value' => $instance['login_image']))
			.show_textfield(array('name' => $this->get_field_name('login_heading'), 'text' => __("Heading", $this->obj_custom_login->lang_key), 'value' => $instance['login_heading'], 'xtra' => " id='".$this->widget_ops['classname']."-title'"))
			.show_textarea(array('name' => $this->get_field_name('login_above_form'), 'text' => __("Content Above Form", $this->obj_custom_login->lang_key), 'value' => $instance['login_above_form']))
		."</div>";
	}
}
...
class widget_registration_form extends WP_Widget
{
	function __construct()
	{
		...
		$this->widget_ops = array(
			...
			'description' => __("Display a Registration Form", $this->obj_custom_login->lang_key)
		);
		...
		parent::__construct('registration-widget', __("Registration Form", $this->obj_custom_login->lang_key), $this->widget_ops);
	}

	function get_roles_for_select()
	{
		$arr_data = array();
		$arr_data[''] = "-- ".__("All", $this->obj_custom_login->lang_key)." --";
	...
	{
		return array(
			'username' => __("Username", $this->obj_custom_login->lang_key),
			'full_name' => __("Full Name", $this->obj_custom_login->lang_key),
			'company' => __("Company", $this->obj_custom_login->lang_key),
		);
	}
		...
		echo $before_widget;
			...
			if($is_allowed)
			{
				if(isset($_POST['btnSendRegistration']))
				{
					...
					{
						$error_text = __("You have to enter both Username and E-mail, then I can process the registration for you", $this->obj_custom_login->lang_key);
					}
					...
					else
					{
						...
						else
						{
							...
							$done_text = __("I processed the registration for you. You should have a message in your inbox shortly, with login information.", $this->obj_custom_login->lang_key);
							$display_form = false;
						}
					}
				}
			...
			{
				$error_text = __("You do not have the rights to view this form", $this->obj_custom_login->lang_key);
				$display_form = false;
			}
			...
			if($display_form == true)
			{
				...
				echo "<form method='post' action='' class='mf_form'>";
					...
					if(in_array('username', $instance['registration_fields']))
					{
						echo show_textfield(array('name' => 'user_login', 'text' => __("Username", $this->obj_custom_login->lang_key), 'value' => $user_login, 'placeholder' => "abc123", 'required' => true));
					}

					echo show_textfield(array('name' => 'user_email', 'text' => __("E-mail", $this->obj_custom_login->lang_key), 'value' => $user_email, 'placeholder' => get_placeholder_email(), 'required' => true));

					if($instance['registration_collect_name'] == 'yes' || in_array('full_name', $instance['registration_fields']))
					{
						echo "<div class='flex_flow'>"
							.show_textfield(array('name' => 'first_name', 'text' => __("First Name", $this->obj_custom_login->lang_key), 'value' => $first_name, 'placeholder' => "Jane", 'required' => true))
							.show_textfield(array('name' => 'last_name', 'text' => __("Last Name", $this->obj_custom_login->lang_key), 'value' => $last_name, 'placeholder' => "Doe", 'required' => true))
						."</div>";
					}
					...
					{
						echo "<div class='flex_flow'>"
							.show_textfield(array('name' => 'profile_company', 'text' => __("Company", $this->obj_custom_login->lang_key), 'value' => $profile_company, 'required' => true))
						."</div>";
					}
					...
					if(is_user_logged_in())
					{
						if(IS_ADMIN)
						{
							...
							if(count($arr_data) > 1)
							{
								echo show_select(array('data' => $arr_data, 'name' => 'role', 'text' => __("Role", $this->obj_custom_login->lang_key), 'value' => $role));
							}
					...
					{
						echo show_checkbox(array('text' => __("I consent to having this website store my submitted information, so that they can contact me if necessary", $this->obj_custom_login->lang_key), 'value' => 1, 'required' => true, 'xtra_class' => "small"));
					}

					echo "<div class='form_button'>"
						.show_button(array('name' => 'btnSendRegistration', 'text' => __("Register", $this->obj_custom_login->lang_key)))
					."</div>";

					if(!is_user_logged_in())
					{
						echo "<p>".__("Do you already have an account?", $this->obj_custom_login->lang_key)." <a href='".wp_login_url()."'>".__("Log In", $this->obj_custom_login->lang_key)."</a></p>";
					}
	...
	function form($instance)
	{
		...
		echo "<div class='mf_form'>"
			...
			.show_textfield(array('name' => $this->get_field_name('registration_heading'), 'text' => __("Heading", $this->obj_custom_login->lang_key), 'value' => $instance['registration_heading'], 'xtra' => " id='".$this->widget_ops['classname']."-title'"))
			.show_select(array('data' => $this->get_roles_for_select(), 'name' => $this->get_field_name('registration_who_can'), 'text' => __("Who Can Register?", $this->obj_custom_login->lang_key), 'value' => $instance['registration_who_can']));
			...
			{
				echo show_select(array('data' => get_yes_no_for_select(), 'name' => $this->get_field_name('registration_collect_name'), 'text' => __("Collect full name from user", $this->obj_custom_login->lang_key), 'value' => $instance['registration_collect_name']));
			}

			echo show_select(array('data' => $this->get_fields_for_select(), 'name' => $this->get_field_name('registration_fields')."[]", 'text' => __("Fields to Display", $this->obj_custom_login->lang_key), 'value' => $instance['registration_fields']))
		."</div>";
	}
}
...
class widget_lost_password_form extends WP_Widget
{
	function __construct()
	{
		...
		$this->widget_ops = array(
			...
			'description' => __("Display a Lost Password Form", $this->obj_custom_login->lang_key)
		);
		...
		parent::__construct('lost-password-widget', __("Lost Password Form", $this->obj_custom_login->lang_key), $this->widget_ops);
	}
	...
	function retrieve_password($login)
	{
		...
		{
			$errors->add('invalidcombo', __("Invalid Username or E-mail", $this->obj_custom_login->lang_key));
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					$message = "<p>"
						.__("Someone has requested a password reset for the following account", $this->obj_custom_login->lang_key).":<br>"
						.__("Username", $this->obj_custom_login->lang_key).": ".$user_login
					."</p>
					<p>".__("If this was a mistake, just ignore this email and nothing will happen.", $this->obj_custom_login->lang_key)."</p>
					<p>"
						.__("To reset your password, visit the following address").":<br>"
						."<a href='".$lost_password_link."'>".$lost_password_link."</a>
					</p>";

					$title = sprintf(__("[%s] Password Reset", $this->obj_custom_login->lang_key), $site_name);
					...
					{
						wp_die(__("The email could not be sent.", $this->obj_custom_login->lang_key)."<br>\n".__("Possible reason: your host may have disabled the mail() function.", $this->obj_custom_login->lang_key));
					}
				}
			}
		}
			...
			switch($action)
			{
				case 'rp':
					...
					if(isset($_POST['btnSendResetPassword']))
					{
						...
						if(!$user || is_wp_error($user))
						{
							...
							{
								$error_text = sprintf(__("The key that you used has expired. Please, %srequest a new key here%s", $this->obj_custom_login->lang_key), "<a href='".wp_lostpassword_url()."'>", "</a>");
							}

							else
							{
								$error_text = sprintf(__("The key that you used was invalid. Please, %srequest a new key here%s", $this->obj_custom_login->lang_key), "<a href='".wp_lostpassword_url()."'>", "</a>");
							}
						}
						...
						else if(!$errors->get_error_code() && $user_pass != '')
						{
							...
							$done_text = __("Your password has been reset", $this->obj_custom_login->lang_key)." <a href='".wp_login_url()."'>".__("Log In", $this->obj_custom_login->lang_key)."</a>";
					...
					if($display_form == true)
					{
						echo "<form method='post' action='".wp_lostpassword_url()."?action=rp' class='mf_form'>"
							.show_password_field(array('name' => 'user_pass', 'text' => __("New Password", $this->obj_custom_login->lang_key), 'value' => $user_pass, 'description' => wp_get_password_hint()));

							do_action('resetpass_form', $user);

							echo "<div class='form_button'>"
								.show_button(array('name' => 'btnSendResetPassword', 'text' => __("Reset Password", $this->obj_custom_login->lang_key)))
				...
				default:
					...
					if(isset($_POST['btnSendLostPassword']))
					{
						if($user_login == '')
						{
							$error_text = __("You have to enter the e-mail address, then I can process the request for you", $this->obj_custom_login->lang_key);
						}
						...
						else
						{
							...
							{
								$done_text = __("I found the account that you were looking for. Please, check your inbox for the confirmation link.", $this->obj_custom_login->lang_key);
					...
					if($display_form == true)
					{
						...
						echo "<form method='post' action='' class='mf_form'>" //".esc_url(network_site_url('wp-login.php?action=lostpassword', 'login_post'))."
							.show_textfield(array('name' => 'user_login', 'text' => __("Username or E-mail", $this->obj_custom_login->lang_key), 'value' => $user_login, 'placeholder' => "abc123 / ".get_placeholder_email(), 'required' => true));

							do_action('lostpassword_form');

							echo "<div class='form_button'>"
								.show_button(array('name' => 'btnSendLostPassword', 'text' => __("Get New Password", $this->obj_custom_login->lang_key)))
							."</div>
							<p>".__("Do you already have an account?", $this->obj_custom_login->lang_key)." <a href='".wp_login_url()."'>".__("Log In", $this->obj_custom_login->lang_key)."</a></p>
						</form>";
					}
				break;
			}
	...
	function form($instance)
	{
		...
		echo "<div class='mf_form'>"
			...
			.show_textfield(array('name' => $this->get_field_name('lost_password_heading'), 'text' => __("Heading", $this->obj_custom_login->lang_key), 'value' => $instance['lost_password_heading'], 'xtra' => " id='".$this->widget_ops['classname']."-title'"))
		."</div>";
	}
}
...
class widget_logged_in_info extends WP_Widget
{
	function __construct()
	{
		...
		$this->widget_ops = array(
			...
			'description' => __("Display Information About the Logged in User", $this->obj_custom_login->lang_key)
		);
		...
		parent::__construct(str_replace("_", "-", $this->widget_ops['classname']).'-widget', __("Logged in Information", $this->obj_custom_login->lang_key), $this->widget_ops);
	}

	function get_user_info_for_select()
	{
		$arr_data = array(
			'name' => __("Name", $this->obj_custom_login->lang_key),
			'role' => " - ".__("Role", $this->obj_custom_login->lang_key),
			'profile' => __("Profile", $this->obj_custom_login->lang_key),
			'logout' => __("Log Out", $this->obj_custom_login->lang_key),
			'image' => __("Image", $this->obj_custom_login->lang_key),
		);
				...
				echo "<div class='logged_in_info section'>";
					...
					if(count($instance['logged_in_info_display']) == 0 || in_array('name', $instance['logged_in_info_display']) || in_array('profile', $instance['logged_in_info_display']) || in_array('logout', $instance['logged_in_info_display']))
					{
						echo "<ul>";
							...
							{
								echo "<li><a href='".get_edit_profile_url()."'>".__("Your Profile", $this->obj_custom_login->lang_key)."</a></li>";
							}

							if(count($instance['logged_in_info_display']) == 0 || in_array('logout', $instance['logged_in_info_display']))
							{
								echo "<li><a href='".wp_logout_url()."'>".__("Log Out", $this->obj_custom_login->lang_key)."</a></li>";
							}
					...
					if(count($instance['logged_in_info_display']) == 0 || in_array('image', $instance['logged_in_info_display']))
					{
						echo "<div>
							<div class='logged_in_avatar'>"
								.get_avatar(get_current_user_id(), 60, '', sprintf(__("Profile Image for %s", $this->obj_custom_login->lang_key), get_user_info()))
							."</div>
						</div>";
					}
	...
	function form($instance)
	{
		...
		echo "<div class='mf_form'>"
			//.show_textfield(array('name' => $this->get_field_name('logged_in_info_heading'), 'text' => __("Heading", $this->obj_custom_login->lang_key), 'value' => $instance['logged_in_info_heading'], 'xtra' => " id='".$this->widget_ops['classname']."-title'"))
